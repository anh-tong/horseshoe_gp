...
class acq_fun(torch.nn.Module):
    def __init__(self, lower_bound, upper_bound, sols, sol_val):
        super(acq_fun, self).__init__()
        self.dim = None
        self.lower_bound = torch.nn.Parameter(torch.Tensor(lower_bound))
        self.upper_bound = torch.nn.Parameter(torch.Tensor(upper_bound))
        self.sols = sols
...
class branin_rcos(acq_fun):
    ...
    def forward(self, x):
        x1 = x[:, 0] * (self.upper_bound[0] - self.lower_bound[0]) + self.lower_bound[0]
        x2 = x[:, 1] * (self.upper_bound[1] - self.lower_bound[1]) + self.lower_bound[1]
...
class six_hump_camel_back(acq_fun):
    ...
    def forward(self, x):
        x1, x2 = x * (self.upper_bound - self.lower_bound) + self.lower_bound
