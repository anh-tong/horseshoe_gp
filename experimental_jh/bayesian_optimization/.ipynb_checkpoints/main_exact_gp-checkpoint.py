...
def acq_max(lb, ub, sur_model, y_max, acq_fun, n_warmup = 10000, iteration = 10):
    ...
    for iterate in range(iteration):
        ...
        """
        res = minimize(lambda x: -acq_fun(x, y_max),
                       tf.reshape(locs, (1, -1)),
                       bounds=bound,
                       method="L-BFGS-B")
...
if __name__ == "__main__":
    bench_functions = [branin_rcos, six_hump_camel_back, hartman_6, goldstein_price, rosenbrock]
    
    ###Result directory
    save_file = "./exact_GP/"
    
    for opt in bench_functions:
        ...
        while num_test < args.num_init:
            try:
                ...
                model.kernel.lengthscales.assign(0.5)
                ...
                for tries in range(args.num_trial):
                    x_new, y_new = acq_max(
                        bench_fun.lower_bound,
                        bench_fun.upper_bound,
                        model,
                        tf.reduce_max(y),
                        acq_fun)
                    ...
                    #Result
                    df_result[tries, num_test] = tf.reduce_min(y, axis=0).numpy()
        ...
        df_result.to_csv(save_file + opt.__name__ + ".csv")
