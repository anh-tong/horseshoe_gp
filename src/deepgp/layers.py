...
class Layer(Module):
    ...
    def sample_from_conditional(self, X, z=None, full_cov=False):
        ...
        if self.input_dim:
            shape = [tf.shape(X)[0], tf.shape(X)[1], self.input_dim]
            X_prop = tf.reshape(X[:, :, :self.input_dim], shape)
...
class SVGPLayer(Layer):
    def __init__(self, kern, Z, num_outputs, mean_function,
                 white=False, input_dim=None, **kwargs):
        """
        ...
        if not self.white:  # initialize to prior
            ...
            self.q_sqrt = Parameter(np.tile(Lu[None, :, :], [num_outputs, 1, 1]), transform=triangular(),
                                    name="q_sqrt")
