...
def create_kernels(data_shape, kernel_order, repetition):
    ...
    return create_rbf(data_shape)


def create_model(inducing_point, data_shape, num_data, n_inducing, kernel_order, repetition):
    kernel = create_kernels(data_shape, kernel_order, repetition)
    likelihood = Gaussian()
    model = SVGP(kernel, likelihood, inducing_variable=inducing_point, num_data=num_data)
...
if __name__ == "__main__":
    ...
    ckpt_dir = "../model/se_{}".format(unique_name)
