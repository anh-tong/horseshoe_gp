...
def create_model_from_sum_kernel(inducing_point, data_shape, num_data, n_inducing, kernel_order, repetition):
    kernels = create_kernels(data_shape, kernel_order, repetition)
    kernel = Sum(kernels)
    likelihood = Gaussian()
    model = SVGP(kernel, likelihood=likelihood, inducing_variable=inducing_point, num_data=num_data)
    return model

def plot_weight_svgp(svgp:SVGP):
    ...
    def get_variance(kernel):
        if isinstance(kernel, Product):
            ...
            return kernel.base_kernel.variance
        else:
            return kernel.variance

    w = [get_variance(k).numpy() for k in kernels]

    plt.figure(figsize=(golden_ratio*2, 1))
    plt.bar(np.arange(24), np.abs(w))
    plt.xticks([])
    plt.xlim(-1, 24)
...
def plot_weights(selector):
    ...
    plt.xlabel(r"kernels $k_i$")
    plt.ylabel(r"$w_i$")
    plt.savefig("../figure/airline_weight_no_prior.png", dpi=300, bbox_inches="tight")
    plt.savefig("../figure/airline_weight_no_prior.pdf", dpi=300, bbox_inches="tight")
...
def plot_abcd(x_train, y_train, x_test, y_test, x_extra, mu, lower, upper):
    ...
    plt.savefig("../figure/airline_no_prior.png", dpi=300, bbox_inches="tight")
    plt.savefig("../figure/airline_no_prior.pdf", dpi=300, bbox_inches="tight")
...
if __name__ == "__main__":
    ...
    # model = create_model_from_sum_kernel(inducing_point,
    #                      data_shape=data_shape,
    #                      num_data=len(y_train),
    #                      n_inducing=n_inducing,
    #                      kernel_order=kernel_order,
